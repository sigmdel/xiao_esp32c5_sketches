...
Because of the Arduino sketch naming constraints, the `main.cpp` file of a project is not stored in the default `src` directory. A `src_dir` entry in the `platformio.ini` confguration file provides the name of the directory in which `main.cpp` is found. 
